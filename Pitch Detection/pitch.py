...
def compute_stft(signal, sr, hop_size=512, win_size=1024):
...
def apply_comb_filter(freqs, spectrum, fundamental_range=(50, 1000)):
...
def detect_notes(signal, sr, seg_times, hop_size=512, win_size=1024):
    note_frequencies = []
    ...
    for i in range(len(seg_times) - 1):
        ...
        D, freqs = compute_stft(segment, sr, hop_size, win_size)
        spectrum = np.mean(np.abs(D), axis=1)
        
        fundamental_freq = apply_comb_filter(freqs, spectrum)
...
def freq_to_note(freq):
    ...
    note_name = librosa.midi_to_note(note_num)
    return note_name 

def plot_comb_filter_response(freqs, spectrum, fundamental_range=(50, 1000)):
...
detected_notes = [freq_to_note(f) for f in detected_frequencies]
